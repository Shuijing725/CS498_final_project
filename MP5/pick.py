...
def plan_pick_one(world,robot,object,gripper,grasp):
    ...
    solved = ik.solve_global(grasp.ik_constraint, feasibilityCheck=lambda: is_collision_free_grasp(world, robot, object))
    ...
    gripper_link = robot.link(gripper.base_link)
    p = gripper_link.getWorldPosition((0,0,0))
    d = gripper_link.getWorldDirection((0,0,1))
    obje = ik.objective(robot.link(gripper.base_link), local=[(0,0,0),(0,0,1)], world=[vectorops.madd(p,d,-0.1), vectorops.madd(p,d,0.9)])
    solved = ik.solve_global(obje, feasibilityCheck=lambda: is_collision_free_grasp(world, robot, object))
    
    qpregrasp = robot.getConfig()   #TODO solve the retraction problem for qpregrasp?
    ...
    for waypoint in transit:
        robot.setConfig(waypoint)
        if not is_collision_free_grasp(world,robot,object):
            return None
            
    # Note: we should also plan a path here, but I sampled 100 points along the path to quickly determine feasibility
    for i in range(100):
        interpolated = vectorops.interpolate(qpregrasp, qopen, i/100.0)
        robot.setConfig(interpolated)
        if not is_collision_free_grasp(world,robot,object):
            return None
    ...
    p = gripper_link.getWorldPosition((0,0,0))
    d = gripper_link.getWorldDirection((0,0,1))
    obje = ik.objective(robot.link(gripper.base_link), local=[(0,0,0),(0,0,1)], world=[vectorops.add(p,(0,0,0.1)), vectorops.add(vectorops.add(p,(0,0,0.1)),d)])
    solved = ik.solve_global(obje, feasibilityCheck=lambda: is_collision_free_grasp(world, robot, object))
    
    qlift = robot.getConfig()
    
    return (
    RobotTrajectory(robot,milestones=[qstart]+transit)
    ,RobotTrajectory(robot,milestones=[qpregrasp,qopen,qgrasp])
...
def plan_pick_grasps(world,robot,object,gripper,grasps):
    ...
    """
    #TODO: implement me
    #Note: time limit is tuned in feasible_plan function inside planning.py
    qstart = robot.getConfig()
    for grasp in grasps:
        robot.setConfig(qstart)
        result = plan_pick_one(world,robot,object,gripper,grasp)
        if result is None:
            continue
        else:
            return result
...
class PickPlanner(MultiStepPlanner):
    ...
    def solve_qgrasp(self,grasp):
        ...
        #TODO solve the IK problem for qgrasp?
        solved = ik.solve_global(grasp.ik_constraint, feasibilityCheck=lambda: is_collision_free_grasp(self.world, self.robot, self.object))
    ...
    def solve_approach(self,grasp,qgrasp):
        ...
        for i in range(100):
            interpolated = vectorops.interpolate(qpregrasp, qgrasp, i/100.0)
            self.robot.setConfig(interpolated)
            if not is_collision_free_grasp(self.world, self.robot, self.object):
                return None
    ...
    def solve_transit(self,qpregrasp):
        ...
        transit = feasible_plan(self.world,self.robot,qpregrasp)
    ...
    def score(self,plan):
        ...
        num_solved = len([k for k in plan if not k.startswith('_')])
        return plan.get('_solve_time',0) - 0.3*num_solved
    ...
    def assemble_result(self,plan):
        ...
        return (
            RobotTrajectory(self.robot,milestones=[qstart]+transit)
            ,RobotTrajectory(self.robot,milestones=approach + [qgrasp])
            ,RobotTrajectory(self.robot,milestones=[qgrasp,qlift])
        )
        return plan
